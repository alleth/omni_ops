...
- **Styling**: Bootstrap 5 for layout plus **Tailwind, which is installed and fully working** — `tailwindcss@3.4.18` is in `dev/node_modules`, `dev/postcss.config.js` loads it, `dev/src/styles/index.css` holds the `@tailwind` directives and is imported by `index.js`, and the build emits a ~78 KB utility bundle. `darkMode: 'class'`, content globs cover `./src/**/*.{js,jsx,ts,tsx}`, and arbitrary values work (`z-[10050]` emits `.z-\[10050\]{z-index:10050}`). **The catch: it is absent from `package.json`.** It survives only because it is physically present in `node_modules`, so a fresh `npm install`/`npm ci` on another machine installs everything *except* Tailwind and all utility styling silently disappears. Run `npm install -D tailwindcss@3.4.18` to pin it properly before anyone clones this. (An earlier version of this file claimed Tailwind was not installed and that the classes were inert — that was wrong.)
...
Key endpoints:
- `GET /api/public/summary.json` — **the only endpoint the public pre-login page uses.** Returns `{success, regions, sites, hardware}` in one round trip. `hardware` is a **column projection**, not the full record: only `site_code`, `item_desc`, `sub_major_type`, `hw_brand_name`, `hw_model`, `os_type`, `hw_status`, `hw_date_acq` — the eight fields `MasterfileLanding.js` actually reads. `PublicController::HARDWARE_FIELDS` is the allowlist; **adding a field there publishes it to anyone who can reach the server**, so treat it as a decision, not a convenience. It replaced three unauthenticated calls to `region-tbl`/`site-list-tbl`/`hw-tbl`, the last of which was serving every column of all ~18,250 rows — serials, asset tags, IPs, MACs, hostnames, usernames — to the world. Payload went 12.5 MB → 3.2 MB raw, ~500 KB → ~85 KB gzipped. Rows are deliberately **not** restricted to `On Site`: every consumer on that page filters by status itself except `availableHardwareTypes`, which needs distinct `item_desc` across all statuses for the aging dropdown. Kept as a projection rather than server-side aggregates on purpose — the page derives sixteen groupings and re-derives them on every region/site filter change, so aggregating server-side would mean two copies of ~600 lines of grouping logic free to drift.
...
`MasterfileLanding.js` (`/masterfile`) is the public pre-login stats page — it renders live hardware summary charts and stats without requiring a session, from the single projected `GET /api/public/summary.json` (see **Backend API**). It is the one page that must keep working with no session at all, so anything that tightens API access has to account for it.
...
7. **A few components bypass `useApi` with host-detected absolute URLs** — do not assume all network calls go through the hook or the CRA proxy:
   - `MasterfileLogin.js` builds its own `API_BASE` via `getApiBase()`, which returns `http://omniops.local` only for `localhost:3000` and `''` (relative, same-origin) everywhere else — correct, but a second implementation of what `useApi` already does.
   - **`MasterfileProfile.js` used to hardcode `http://omniops.local` unconditionally for its profile-update `fetch`, which broke Save on the VM entirely** (fixed 2026-09-18, now relative). That hostname only resolves on machines with the hosts-file entry, so for anyone using the production VM at `http://192.168.4.95:8888` the request never reached the server — it failed in the browser, hit the `catch`, and showed the generic "Failed to save profile. Please try again." while the backend was working perfectly. **Never hardcode a backend host in a request path**: relative URLs work everywhere (same-origin in production, CRA proxy in dev). Note this kind of failure is invisible from the server side — there is nothing in `logs/error.log` because no request arrives.
...
- **`request_tbl.hw_id` was missing entirely from the VM's database until 2026-09-02** — a distinct bug from the schema-cache gotcha above, and worth telling apart from it. `hw_id` had been added to the local dev DB by hand (no migration file ever existed for it), so `migrations migrate` on the VM had nothing to apply and the column was never created there at all — not stale-cached, genuinely absent (confirmed by deleting the VM's `tmp/cache/models/myapp_cake_model_default_request_tbl` and re-reading: `hw_id` still didn't appear in any of the 79 live request rows, even freshly reflected). Effect: `RequestTblController::add()`'s per-item `'hw_id' => $itemData['hw_id']` silently never persisted, so every request in production has `hw_id = NULL`; the client-side revert-to-`On Site` on cancel/reject (`updateHardwareStatusForRequest` in `requestActions.js`) is gated on `request.hw_id` being truthy, so it was a permanent no-op — hardware whose request got rejected/canceled stayed stuck at `Pending` forever instead of reverting, invisible in every On Site view with no request left in `request_tbl` to explain why. `config/Migrations/20260902150000_AddHwIdToRequestTbl.php` tracks the column. **It applied on the VM on 2026-09-18 — `request_tbl.hw_id` now exists there.**
  ...
  - **That single unresolved row was a false alarm, and the cause is a quirk in the script.** `hw_id 6157` does have a request — #84, still PENDING — so its `Pending` status was legitimate all along. Final tally: **172 correctly pending, 3 reverted, 0 needing manual work.** The script missed it because its asset/serial fallback requires *both* `asset_num` AND `serial_num` to match when both are usable, and it trims only the `hw_tbl` side (`trim($hw->hw_serial_num)`) against an untrimmed `request_tbl.serial_num`. MySQL ignores trailing spaces in comparisons but **not leading** ones, and that row's stored serial is `' PA63J17LtY'`. If a future run reports UNRESOLVED, check before acting: `SELECT request_id, status, asset_num, serial_num FROM request_tbl WHERE TRIM(asset_num)='...' OR TRIM(serial_num)='...';` — a PENDING/APPROVED match means leave it alone.
  - **Stray leading/trailing whitespace in `hw_asset_num` / `hw_serial_num` is common** (297 asset numbers and 15 serials in the dev DB). The app itself is safe — `MasterfileInventory`'s duplicate detection trims both sides — but any *raw SQL* comparing these columns must `TRIM()` both sides or it will silently miss rows. Same class of bug as the Inventory Type filter dropping `item_desc` values with trailing whitespace (commit 92f4cc1).
...
- **Anything that must appear above a modal has to be portaled too — raising its z-index alone does nothing.** `#modal-root` is appended to `<body>`, after the app root, so an element rendered inline in the page tree loses on stacking order no matter what z-index it carries. This bit the Inventory toast: the duplicate asset/serial warning fires *while* the Add Hardware modal is still open (it deliberately stays up so the values can be corrected), and at inline `z-[60]` it rendered underneath that modal's `bg-black/60 backdrop-blur-sm` overlay — dimmed and blurred to the point of being unreadable, exactly when the user needed to read it. Same defect existed in `MasterfileUsers` (inline `z-50` toast vs portaled `z-50` modals), hiding "Username is already taken" and failed password resets. Both now portal into `#modal-root` at `z-[10050]`. **Layer ordering**: modals `z-[9999]`, `RequestDetailModal`'s reject confirmation `z-[10000]`, toasts `z-[10050]` — keep toasts on top.
