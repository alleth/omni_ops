...
- **`request_tbl.hw_id` was missing entirely from the VM's database until 2026-09-02** — a distinct bug from the schema-cache gotcha above, and worth telling apart from it. `hw_id` had been added to the local dev DB by hand (no migration file ever existed for it), so `migrations migrate` on the VM had nothing to apply and the column was never created there at all — not stale-cached, genuinely absent (confirmed by deleting the VM's `tmp/cache/models/myapp_cake_model_default_request_tbl` and re-reading: `hw_id` still didn't appear in any of the 79 live request rows, even freshly reflected). Effect: `RequestTblController::add()`'s per-item `'hw_id' => $itemData['hw_id']` silently never persisted, so every request in production has `hw_id = NULL`; the client-side revert-to-`On Site` on cancel/reject (`updateHardwareStatusForRequest` in `requestActions.js`) is gated on `request.hw_id` being truthy, so it was a permanent no-op — hardware whose request got rejected/canceled stayed stuck at `Pending` forever instead of reverting, invisible in every On Site view with no request left in `request_tbl` to explain why. `config/Migrations/20260902150000_AddHwIdToRequestTbl.php` tracks the column so it will apply on the next `migrations migrate`. **As of 2026-09-18 that migration has still NOT run on the VM — `request_tbl.hw_id` genuinely does not exist there.**
  ...
  - **`scripts/backfill_revert_stuck_pending_hw_status.php` cannot run on the VM until the migration does** — it queries `request_tbl.hw_id` directly and dies with the `Unknown column` error above. Run the migration first, then the backfill. It matches via `hw_id` *or* an asset/serial fallback, so it still repairs rows that predate the column; it is safe to re-run and supports `--dry-run`.
...
8. **Still outstanding as of 2026-09-18**: `20260902150000_AddHwIdToRequestTbl` has *not* applied on the VM — `request_tbl.hw_id` does not exist there, confirmed by a live `SQLSTATE[42S22] Unknown column 'hw_id'` from the backfill script. Run `php bin/cake migrations migrate` **on the VM itself**, verify with `SHOW COLUMNS FROM request_tbl LIKE 'hw_id';`, then delete `tmp/cache/models/myapp_cake_model_default_request_tbl` so Apache re-reads the schema, then run `scripts/backfill_revert_stuck_pending_hw_status.php --dry-run` followed by a real run. Remove this step once `SHOW COLUMNS` returns a row. See Key Domain Details for why this keeps getting mistaken for a stale cache.
9. Confirm no executable files sit in any upload directory on the target, and that both guards are in place:
   ```bash
