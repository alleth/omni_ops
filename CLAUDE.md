...
- Hardware with an active PENDING pull-out/relocation request is temporarily hidden from `MasterfileInventory`'s On Site list (`pendingRequestHwIds` in `MasterfileInventory.js`) — `hw_status` itself only changes on APPROVAL (see below), so without this the same unit could be selected for a second bulk request while the first is still pending. Rejecting/canceling drops the request out of PENDING and the hardware reappears on the next fetch automatically.
- `dev/src/utils/requestActions.js` centralizes request approve/cancel/delete plus approval's hardware side effects (shared by `RequestDetailModal` and the Dashboard's bulk actions, so the logic can't drift between single and bulk entry points): `approveRequestCore` sets status → APPROVED, flips the hardware's `hw_status` to `Pullout`, and — for RELOCATION — duplicates the hardware record at `destination_site` as a new `On Site` row (`POST /api/hw-tbl/add.json`) via `duplicateHardwareForRelocation`. It deliberately skips attachment replacement (that stays a single-request, SPV-specific step in `RequestDetailModal`).
