...
8. **One-time, as of 2026-09-02**: `20260902150000_AddHwIdToRequestTbl` has never actually reached the VM (confirmed live on 2026-09-02 — `request_tbl.hw_id` was absent from every request row there, not just cached stale; see Key Domain Details). The next `migrations migrate` on the VM will add it — after it lands, clear `tmp/cache/models/myapp_cake_model_default_request_tbl` (step 6) and run `scripts/backfill_revert_stuck_pending_hw_status.php` to un-stick hardware left at `Pending` by requests that went terminal before the column existed. Remove this step once done.
